...
def read_file(file):
    with open(file, 'r', encoding='utf-8', newline='') as p:
        ...
        for line in p:
            ...
            line = re.sub(r'\s*<\|begin_of\s+', '', line)
...
def basic_stats(sentences):
    ...
    for t in sentences:
        for tt in t:
            types.add(tt[0])
            tokens.append(tt[1])
    return {'types' : len(types), 'tokens' : len(tokens), 'mean_token' : np.mean(tokens), 'median_token' : np.median(tokens), 'sd_token' : np.std(tokens)}

def main(file):
    return sp_words_with_scores(read_file(file))

def main_basic_stats(files):
    filenames = glob.glob(files)
    for filename in filenames:
        print(filename)
        print(basic_stats(sp_words_with_scores(read_file(filename))))
    # return basic_stats(sp_words_with_scores(read_file(file)))

def main_csv(files, outputfile):
    filenames = glob.glob(files)
    stats = []
    with open(outputfile, 'w', newline='') as csvfile:
        fieldnames = ['filename', 'tokens', 'types', 'mean_token', 'median_token', 'sd_token']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        for filename in filenames:
            print(filename)
            statistics = basic_stats(sp_words_with_scores(read_file(filename)))
            filename = re.sub(r'_surprisal.*', '', filename)
